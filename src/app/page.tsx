...
export default async function HomePage() {
  ...
  return (
    <StorefrontShell>
      ...
      {/* Category circles / cards — reference uses collection tiles */}
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-6xl px-4">
          <div className="mb-8 text-center">
            ...
            <p className="mt-1 text-sm text-[#747474]">Find everything you want — بالعربية: اعثري على كل ما تبحثين عنه</p>
          </div>
                ...
                <Link href={`/product/${p.slug}`} className="block">
                  ...
                  <CardContent className="space-y-2 p-4">
                    ...
                    <div className="flex flex-wrap items-baseline gap-2">
                      <span className="text-lg font-semibold text-[#00BF0E]">{formatMad(p.price)}</span>
                      {p.compareAtPrice ? (
      ...
      {/* Spotlight + COD-style form (reference home block) */}
      {spotlight ? (
                ...
                <div className="flex items-baseline gap-2">
                  {spotlight.compareAtPrice ? (
                    <span className="text-lg text-[#696969] line-through">
                      {formatMad(spotlight.compareAtPrice)}
                    </span>
                  ) : null}
                  <span className="text-2xl font-bold text-[#00BF0E]">{formatMad(spotlight.price)}</span>
                </div>
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-6xl px-4">
          <div className="mb-8 text-center">
            ...
            <p className="mt-1 text-sm text-[#747474]">Find everything you want</p>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {promos.map((p) => (
              <Card key={p.id} className="overflow-hidden border border-[#f0f0f0] shadow-none">
                <Link href={`/product/${p.slug}`} className="block">
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="mt-2 text-lg font-semibold text-[#00BF0E]">{formatMad(p.price)}</div>
                  </CardContent>
                </Link>
              </Card>
            ))}
